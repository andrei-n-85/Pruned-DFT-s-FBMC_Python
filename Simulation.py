...
K_OFDM = 14
# Number of OFDM symbols (without CP, to have the same data rate as FBMC!)
K_OFDM_noCP = 15
# Number of FBMC symbols, must be a multiple of 2
K_FBMC = 30
# Sampling rate in Hz, should be a multiple of 15e3 and 14 (due to CP).
# Furthermore, fs/F>=L and fs/F/2 an integer.
fs = 15e3 * 14 * 96
# Cyclic prefix length in seconds (for OFDM).
T_CP = 1 / F / 14
# Number of CP subcarriers (for pruned DFT spread FBMC,
# can usually be set to zero => no efficiency loss). Must be even.
L_CP = 0

# Power delay profile: 'TDL-A_xxns', 'TDL-B_xxns', 'TDL-C_xxns', with 'xx' being the rms delay spread in ns.
# Further choices 'VehicularA', 'PedestrianA', 'Flat', 'AWGN'
pdp = "AWGN"
# Velocity in km/h
v_kmh = 200
...
nr_rep = 100
# SNR values
SNR_dB = np.linspace(0, 30, 21)
# If set to true, save Figure 2-5 in "./png/
save_png = False
...
channel = DoublySelectiveChannel(pdp,
                                 # Power delay profile: 'TDL-A_xxns', 'TDL-B_xxns', 'TDL-C_xxns', with 'xx' beeing the rms delay spread in ns. Further choices 'VehicularA', 'PedestrianA', 'Flat'
                                 v_kmh / 3.6 * fc / 2.998e8,  # Maximum doppler shift
                                 fs,
                                 # Sampling rate (Hz). Should approximately match the predefined power delay profile
...
time_pos_mid_OFDM = np.arange(ofdm.nr_symbols_in_time) * (ofdm.imp_CP_length + ofdm.imp_FFTsize) + \
                    ofdm.imp_CP_length + int(ofdm.imp_FFTsize / 2)
time_pos_mid_OFDMnoCP = np.arange(ofdm_noCP.nr_symbols_in_time) * (ofdm_noCP.imp_CP_length + ofdm_noCP.imp_FFTsize) \
                        + ofdm_noCP.imp_CP_length + int(ofdm_noCP.imp_FFTsize / 2)
time_pos_mid_FBMC = np.arange(fbmc.nr_symbols_in_time) * (int(fbmc.imp_FFTsize / 2)) + \
                    int((fbmc.imp_FFTsize * fbmc.imp_overlapping_factor) / 2)
...
for i_rep in range(nr_rep):
    ...
    for i_snr in range(SNR_dB.size):
        ...
        e_OFDM = 1 / h_OFDM
        e_SCFDMA = np.conj(h_OFDM) / (np.abs(h_OFDM) ** 2 + Pn) * 1 / np.mean(1 / (1 + Pn / np.abs(h_OFDM) ** 2),
                                                                              axis=0)
        e_OFDMnoCP = 1 / h_OFDMnoCP
        e_SCFDMAnoCP = np.conj(h_OFDMnoCP) / (np.abs(h_OFDMnoCP) ** 2 + Pn) * 1 / np.mean(
            1 / (1 + Pn / np.abs(h_OFDMnoCP) ** 2), axis=0)
        e_FBMC = 1 / h_FBMC
        e_pDFTsFBMC = np.conj(h_FBMC) / (np.abs(h_FBMC) ** 2 + Pn) * 1 / np.mean(1 / (1 + Pn / np.abs(h_FBMC) ** 2),
                                                                                 axis=0)
    ...
    PAPR_OFDM[i_rep, :] = np.max(
        np.abs(s_OFDM.reshape((ofdm.imp_FFTsize + ofdm.imp_CP_length), K_OFDM, order='F')) ** 2, axis=0)
    PAPR_SCFDMA[i_rep, :] = np.max(
        np.abs(s_SCFDMA.reshape((ofdm.imp_FFTsize + ofdm.imp_CP_length), K_OFDM, order='F')) ** 2, axis=0)
    cutoff = int((fbmc.imp_overlapping_factor - 0.5) * fbmc.imp_FFTsize / 2)
    PAPR_FBMC[i_rep, :] = np.max(
        np.abs(s_FBMC[cutoff:-cutoff].reshape(fbmc.imp_FFTsize, int(K_FBMC / 2), order='F')) ** 2, axis=0)
    PAPR_pDFTsFBMC[i_rep, :] = np.max(
        np.abs(s_pDFTsFBMC[cutoff:-cutoff].reshape(fbmc.imp_FFTsize, int(K_FBMC / 2), order='F')) ** 2, axis=0)
    ...
    if i_rep == 0 or ((i_rep + 1) % 10) == 0:
        print("%3d" % (int((i_rep + 1) / nr_rep * 100)) + '% ... ' + str(
            round(toc / (i_rep + 1) * (nr_rep - i_rep - 1) / 60, 1)) + 'minutes left')
...
t_FBMC = np.arange(fbmc.nr_samples) * fbmc.phy_dt - (
            (fbmc.imp_overlapping_factor - 0.5) * fbmc.imp_FFTsize / 2) * fbmc.phy_dt
...
print("OFDMnoCP:   " + "%8d Bits/s" % (
        bitstream_OFDMnoCP.size / (ofdm_noCP.nr_symbols_in_time * ofdm_noCP.phy_time_spacing)))
print("SCFDMAnoCP: " + "%8d Bits/s" % (
        bitstream_OFDMnoCP.size / (ofdm_noCP.nr_symbols_in_time * ofdm_noCP.phy_time_spacing)))
print("FBMC:       " + "%8d Bits/s" % (bitstream_FBMC.size / (fbmc.nr_symbols_in_time * fbmc.phy_time_spacing)))
print("pDFTsFBMC:  " + "%8d Bits/s" % (
        bitstream_pDFTsFBMC.size / (fbmc_trunc.nr_symbols_in_time * fbmc_trunc.phy_time_spacing)))
print("========================================")
